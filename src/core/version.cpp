...
std::optional<SemVer> parseSemVer(const std::string& text) {
    ...
    for (const char c : s) {
        if (c == '.') {
            if (!digitsInPart) return std::nullopt;  // segment vide (".", "1..2")
            if (index == 2) return std::nullopt;     // plus de 3 segments
            ++index;
            digitsInPart = false;
        } else if (c >= '0' && c <= '9') {
            parts[index] = parts[index] * 10 + (c - '0');
            if (parts[index] > 100000000) return std::nullopt;  // garde-fou overflow
            digitsInPart = true;
        } else {
    ...
    if (index != 2 || !digitsInPart) return std::nullopt;  // pas exactement 3 segments pleins
