...
static void registerHotkeys(void)
{
	g_hkToggleAuto = obs_hotkey_register_frontend(
		"streamdirector.toggle_auto",
		"StreamDirector : activer/desactiver le pilotage auto", hkToggleAuto, nullptr);
	g_hkForceWide = obs_hotkey_register_frontend(
		"streamdirector.force_wide", "StreamDirector : forcer le plan large", hkForceWide,
		nullptr);
	...
	for (int i = 0; i < kMaxSpeakerHotkeys; ++i) {
		...
		g_hkForceSpeaker[i] = obs_hotkey_register_frontend(name, desc, hkForceSpeaker,
								   &g_speakerHkCtx[i]);
	}
